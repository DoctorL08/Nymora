...
**SESSION 27 mai 2026 (😀 ÉMOTES EN JEU (E1+E2) + BULLES DE CHAT HUB (C1) + RELAI CLAN + MAJ 0.1.4) :** ajout des émotes de classe + bulles de chat au-dessus des avatars, fix du relai chat clan (backend prod), et publication client **0.1.4**. Client Unity **commité localement puis poussé GitHub** (fin de session). Backend **déployé prod**. 100% View → **pas de bump CombatRulesVersion**.

- **Émotes de classe E1 (local) ✅ commit `bb97fee`** : 15 PNG chibis (5 classes × 3, `Assets/_Nymora/Art/UI/Emotes/`) importés + `EmoteCatalog.asset` via **`Nymora > Setup > Emotes > Import Emotes & Build Catalog`** (parse `Emote_XX_yyy.png`, XX = CS/GH/NC/NS/SR → Colossar/Ghostra/Necram/Nightseer/Soulrender). Bouton émote `:)` à droite du hamburger (`HubMenuShell.Emotes.cs`, partial) + popup des **3 émotes de la classe équipée** (`HubAvatar.Local.NetClassId`, **zéro backend**). `EmoteBubbleView` = bulle sprite world-space (SpriteRenderer rounded + liseré + queue, DA menu) au-dessus de l'avatar, layer "Personnages" order 30000, hauteur `HeadOffsetY=1.30`. Gating = classe courante (toutes les classes sont dispo à tous les joueurs). Distinct des émotes PAYANTES boutique (`emote_taunt`/`emote_bow`).
- **Émotes réseau E2 (RPC Fusion) ✅ commit `4a6444d`** : `HubAvatar.PlayEmote(id)` (owner = State Authority en Shared Mode) → **`[Rpc(RpcSources.StateAuthority, RpcTargets.All)] RpcPlayEmote(NetworkString<_32>)`** (InvokeLocal) → chaque client résout l'id via `_emoteCatalog` (ref simple sur le prefab `HubAvatar`, câblée automatiquement par l'outil Import Emotes) et affiche la bulle. **1re RPC du projet. AUCUN [Networked] field → pas de régén** (le piège "Invalid Length" ne concerne que les [Networked]). Validé multi-instance (éditeur + standalone). RPC non bufferisée = les late-joiners ne voient pas une émote passée (normal).
- **Bulles de chat C1 (hub only) ✅ commit `bb97fee`** : `ChatBubbleView` = bulle texte world-space (Canvas WorldSpace, auto-dimensionnée via `GetPreferredValues`, DA menu, police Ari exposée via `HubMenuShell.MenuFont`, `FontSize=44`, `MaxTextWidth=440`). `HubChatBubbleRouter` (instancié par `HubBootstrap` → vit/meurt avec la scène hub = **pas de bulles en combat**) écoute `HubChatClient.OnMessageReceived` **canal `global` UNIQUEMENT** (pas clan/MP) et route vers l'avatar via `NetDisplayName` (nouveau registre statique `HubAvatar.All`, local + distants). **Pas de RPC : le backend diffuse déjà chaque CHANNEL_MESSAGE à tous les clients du canal, expéditeur inclus** → chaque client mappe à sa copie locale de l'avatar.
- **Relai de chat clan ✅ DÉPLOYÉ PROD (commit backend `e1303bc`)** : 🐛 les canaux `clan:<clanId>` n'existaient pas côté backend (`ChannelRegistry` = liste blanche `global`/`system`) → `JOIN_CHANNEL clan:*` renvoyait `UNKNOWN_CHANNEL`, le membre n'était jamais dans le canal, donc `SEND_MESSAGE` clan ne revenait à personne (pas même l'émetteur). Fix : `channels.join` **crée les canaux `clan:*` à la volée** (+ cleanup quand vide, sur `leave` et `removeClient`) ; `channels.isInChannel` ; handler `JOIN_CHANNEL` clan → **vérif d'appartenance async via `prisma.clanMember` (anti-écoute)** ; handler `SEND_MESSAGE` clan → refus `CLAN_FORBIDDEN` si l'émetteur n'est pas dans le canal (anti-injection). **Relai temps réel, PAS d'historique persistant.** Validé après reconnexion client. Aucune modif client Unity pour ce point.
- **Fix tooltip avatar sans clan** : le ruban (bouts `_endL`/`_endR`) ET les liserés haut/bas (couleur `OrnamentColor` du clan) s'affichaient même sur un perso **sans clan** → masqués via `hasClan` dans `HubAvatarHoverTooltip.RefreshLayout`. (Fait partie du WIP bannières, commité cette session.)
- **MaJ client 0.1.4 publiée en prod** : `nymora-0.1.4.zip` (69 341 367 o, sha256 `e71148192d9048df79b13b8f27c41468328de4d82e5841741b34379d47006f84`) scp → `/opt/nymora-backend/downloads/`, `version.service.ts` bumpé (`CURRENT_CLIENT_VERSION` 0.1.3→0.1.4 + `LATEST_ZIP_FILENAME` + `LATEST_ZIP_SHA256` **ENSEMBLE**), commit+push+`deploy.sh`. `MIN_CLIENT_VERSION` reste `0.1.0` (additif, pas de breaking change). Procédure cf [[project-launcher-publish-workflow]].
- **PROCHAIN STEP** : à décider. Émotes complètes (E1+E2) + bulles chat hub. Pistes : émotes en **combat** (canal View séparé hors sim Quantum), historique chat clan persistant (DB), ou autre polish.

---

**SESSION 26 mai 2026 ter (🏆 CHANTIER CONTENU SUCCÈS + BP — BRIQUE S-A LIVRÉE EN PROD) :** lancement de l'extension contenu (succès 12→200, BP déjà à 100 tiers mais récompenses filler à redesigner). Direction design verrouillée (cf [[project-achievements-bp-content-design]]) : succès = mix paliers+uniques, points+Nymos sur tous + cosmétique sur gros milestones ; cosmétiques **designer-first** (titres+Nymos produisibles maintenant, skins/emotes/**bannières** = placeholder Kyami ; ⚠️ bannières cosmétiques ≠ bandeau de clan). **100% backend.**

- **Brique S-A — Socle récompenses succès ✅ DÉPLOYÉE PROD (commit `fa8a0af`)** : `AchievementDef` gagne `nymos` (crédité au unlock) + `cosmeticReward?` (slot designer-first référençant un `cosmeticId` du catalogue boutique). Helper `grantUnlockRewards` (`achievements/service.ts`) : crédite les Nymos via `wallet.awardCurrency` **idempotent** (clé `ach_<user>_<id>`) + octroi cosmétique **tolérant** (si `cosmeticId` absent du catalogue → log "deferred", Nymos quand même crédités, possession à rétro-octroyer). WS `ACHIEVEMENT_UNLOCKED` transporte désormais `nymos` + `cosmetic.label`. Les 12 succès backfillés (Nymos + slot placeholder `title_veteran_nymora`/`title_maitre_nymora` sur Vétéran/Maître). **Validé E2E sur prod** (test self-cleaning : first_match +50, first_victory +75 → wallet 125 ; reach_lv5 +100). Pas de migration DB.
- **Brique S-B — Contexte de fin de match enrichi + maîtrise par classe ✅ DÉPLOYÉE PROD (commit `9030df3`)** : `onMatchEnd(userId, result)` → `onMatchEnd(userId, ctx)` avec `MatchEndContext { result, classId?, stats? }`. `classId` threadé depuis les 2 callers (settle ranked + route progression). **+20 succès de maîtrise par classe** générés (Pratiquant 25 joués / Initié 10 / Expert 50 / Maître 100 victoires × 5 classes ; palier Maître porte un slot cosmétique titre placeholder) → catalogue **12 → 32**. `MatchCombatStats` (dégâts/sorts/tours) défini comme **contrat client DORMANT** (consommé en S-C ; non vérifié server-side = double-accord alpha). Validé E2E prod (catalog=32, play/win Soulrender progressent). Pas de migration, pas de manip Unity.
- **Batch contenu #1 — Niveaux par classe + fix ranked level-up ✅ DÉPLOYÉ PROD (commit `f3dcf90`)** : 🐛 **FIX** le settle ranked ne déclenchait jamais `onLevelUp` (alors que XP = ranked-only en prod) → les succès `reach_lvN_any` étaient morts ; corrigé. `onLevelUp(userId, level)` → `onLevelUp(userId, level, classId?)` : paliers any + par classe + **Polymathe** (5 classes au même palier, lu depuis `ClassProgression`). **+18 succès** (Adepte/Virtuose/Légende ×5 + Polyvalent/Érudit/Grand Maître, ce dernier avec slot cosmétique titre placeholder) → catalogue **32 → 50**. Validé E2E prod. Pas de migration.
- **Batch contenu #2 — Ranked ✅ DÉPLOYÉ PROD (commit `e7ea65c`)** : `onRankedMatch(userId, newMmr)` appelé au settle pour chaque joueur → matchs classés (`ranked_first/25/100`) + rangs atteints **Or→Légende** (`rank_<slug>`, idempotent via `resolveRank`, pas de skip). **+9 succès** (catégorie `combat` car le menu Unity ne rend que first_steps/combat/progression ; Légende = slot cosmétique titre placeholder) → catalogue **50 → 59**. `rank.service` exporte désormais `RANK_TIERS` + `rankSlug`. Validé E2E prod (rank_or à MMR 1250). Pas de migration.
- **Batch contenu #3 — Économie ✅ DÉPLOYÉ PROD (commit `26f8775`)** : `onNymosEarned` (settle ranked, par montant gagné) → `earn_nymos_1000/10000/50000/100000` ; `onNymosSpent` (achat boutique en Nymos) → `spend_nymos_1000/10000/50000`. **+7 succès** (catégorie `progression` ; Magnat 100k = slot cosmétique titre placeholder) → catalogue **59 → 66**. Pas de dépendance circulaire (shop/service → achievements/service → shop/catalog). Validé E2E prod. Pas de migration.
- **Batch contenu #4 — Collection ✅ DÉPLOYÉ PROD (commit `de32ffc` + fix test `8dd7d0c`)** : `onDeckCreated` (POST /decks) → `deck_builder_1/5/15` ; `onCosmeticAcquired` (achat boutique) → `collect_5/15/30`. **+6 succès** (catégorie `progression` ; Collectionneur 30 = slot cosmétique titre placeholder) → catalogue **66 → 72**. Validé E2E prod. Pas de migration. (NB : coquille compteur test 66→72 oubliée puis corrigée — le test prod l'a attrapée.)
- **Batch contenu #5 — Engagement ✅ DÉPLOYÉ PROD (commit `0751b67`)** : `onQuestClaimed` (claim quête) → `quest_claim_10/50/150` ; `onBpTierReached` (tierUp dans `addBpXp`) → `bp_tier_10/50/100` (idempotent). **+6 succès** (catégorie `progression` ; Increvable + Passe complété = slots cosmétiques titre placeholder) → catalogue **72 → 78**. Validé E2E prod. Pas de migration.
- **Brique S-STATS.a — Succès de combat (backend) ✅ DÉPLOYÉ PROD (commit `89ccacb`)** : `report-result` accepte un objet `stats` optionnel (damageDealt/damageTaken/spellsCast/turns) → threadé via le registre double-accord (`statsByUser`) → settle → `onMatchEnd(ctx.stats)` par joueur. Consommé : `flawless_win`/`flawless_win_10` (0 dégât subi), `damage_10000/100000/500000`, `spells_100/1000`, `swift_win`/`swift_win_10` (≤5 tours). **+9 succès** (catégorie `combat` ; Intouchable + Cataclysme = slots cosmétiques placeholder) → catalogue **78 → 87**. **DORMANTS** jusqu'à S-STATS.b. Non vérifié server-side = alpha double-accord. Validé E2E prod (appel direct `onMatchEnd` avec stats). Pas de migration.
- **Brique S-STATS.b — Envoi des stats de combat (Unity) ✅ LIVRÉ (commit LOCAL `8f0f6d3`, à build+tester par Lorenzo)** : `CombatStatsCollector` (Combat/View, auto-instancié comme CombatAudioView) poll la Frame Quantum (`Filter<Combatant>` + `CombatState.TurnNumber`) → accumule pour le joueur local dégâts infligés/subis, sorts lancés, tours. **100% View, zéro modif sim → pas de bump CombatRulesVersion.** `MatchBridge` porte les stats (`StatDamageDealt/Taken/SpellsCast/Turns` + `HasCombatStats`), `HubMatchResultDisplay` les envoie au report ranked (DTO `RankedReportBodyWithStats` + overload `ReportRankedResultAsync`). Allume les 9 succès de combat de S-STATS.a. ✅ **VALIDÉ E2E EN PROD (build Lorenzo, match classé Ghostra)** : log `Report ranked AVEC stats : dealt=1500 taken=0 spells=9 tours=4` → `flawless_win` + `swift_win` débloqués (+ damage/spells en progression, + cascade ranked/maîtrise/économie/quêtes). Les 87 succès sont pleinement opérationnels en conditions réelles. Commit Unity local `8f0f6d3` (push fin de session).
- **Batch prestige (endgame) ✅ DÉPLOYÉ PROD (commit `27561d0`)** : 8 succès haut-palier réutilisant les triggers existants (zéro nouveau système, zéro modif client). Combat : `wins_250`/`wins_500` (Conquérant = titre), `matches_500`, `ranked_500`, `damage_1000000` (Force dévastatrice = titre), `spells_5000`. Progression : `earn_nymos_250000` (Nabab = titre), `spend_nymos_100000`. Wiring dans `onMatchEnd`/`onRankedMatch`/`onNymosEarned`/`onNymosSpent`. Catalogue **92 → 100**. Validé E2E prod (`test:achievements` → 100 entries, PASSED). **Pas de migration.**
- **Titres-récompense (texte) RÉELS ✅ DÉPLOYÉ PROD (commit `a762502`/`19ad535`)** : les titres étant du texte pur (zéro dépendance Kyami), `shop/catalog.ts` expose `REWARD_TITLES` (21 titres) — tous les `title_*` des succès + les 2 titres gratuits du BP (`bp_title_season`/`bp_title_season_prestige`). Résolus par `getCatalogItem` (donc `grantUnlockRewards` succès + `claimTier` BP créent un **vrai `UserCosmetic`**, fini le « différé »), mais **absents du listing boutique** (`getShopForUser` n'itère que `SHOP_CATALOG`) et **non achetables** (aucun prix → `NOT_PURCHASABLE_WITH_CURRENCY`). Validé E2E prod : `title_intouchable` octroyé via `flawless_win_10`, `bp_title_season` octroyé au claim BP T50 ; skins/bannières/emotes restent designer-first (toujours différés). NB : `Profile` créé dès l'inscription (`auth.ts`) → les séries de victoires fonctionnent dès le 1er match.
- **Bannières-emblèmes (tooltip + boutique) ✅ PROD (`552feea`→`2f9c611`)** : Lorenzo a fourni 5 emblèmes peints (`Downloads/banniere.png`) → découpés/normalisés (Pillow) → `Assets/_Nymora/Resources/UI/Cosmetics/Banners/`. **Affichage** : `HubAvatarHoverTooltip` rend l'emblème équipé centré au-dessus de la plaque (calé par hauteur via `FitHeight`, `BannerHeightFactor=2.8`, `BannerOverlapFactor=0.22`, padding haut élargi si bannière pour ne pas mordre sur le nom de clan). Source de la bannière = `HubAvatar.LocalEquippedBannerId` (résolu dans `RefreshEquippedSkinAsync`) → **local-only pour l'instant** (les remotes nécessiteront un `[Networked] NetBanner` + regen prefab/scène = brique séparée). **Boutique** : décision Lorenzo = items boutique dédiés (PAS récompenses BP), ids propres `banner_lune/parchemin/cristaux/orbe/couronne`, vendus en **Nymos** (2500→6000 selon rareté, `rotating:false`), `previewKey=UI/Cosmetics/Banners/<id>` (vignette via `Resources.Load<Sprite>`). Placeholders `banner_crimson/azure` retirés. Slots `bp_banner_*` du Battle Pass redeviennent designer-first (différés). Dev : script `cosmetic:grant <pseudo> <id> [equip]`. ⏳ **Commit client Unity à pousser en fin de session** (assets renommés + tooltip/HubAvatar). Reste à valider visuellement les 5 en jeu.
- **Audit + fix icônes menu hub (27 mai) — Unity, commit en attente** : audit complet de tout l'UI du menu (shell + Personnage/Cosmétiques/Boutique/Battle Pass/Progression/Social) croisé avec `Resources/UI/Icons/`. **Corrigé** : (1) Boutique — icône de type devant chaque en-tête de catégorie (`HubMenuShop`) ; (2) Boutique — preview des cartes sans asset (titres) → repli sur `ui_icon_type_*` ; (3) Cosmétiques — vignette de ligne idem ; (4) Battle Pass — filigrane récompense (icône Nymos / type cosmétique déduit du label) dans les cellules ; (5) `ui_icon_premium` enfin câblée (coin des cellules premium verrouillées) ; (6) Quêtes — récompense Nymos en sprite `ui_icon_nymos` au lieu du glyphe `◆`. **Fix tofu `✓` (police Ari sans U+2713)** : « Équipé » (Cosmétiques), « Réclamée » (Quêtes), « Réclamé » (BP) passent au sprite `ui_icon_check` (le `✓` s'affichait en carré). **Restant** : 🟠 blason de clan (`ui_clan_emblem_*`/`ui_clan_shape_*`/`ui_banner_medal_*` = sets entiers non câblés ; header clan = carré de couleur → brique dédiée) ; ⚪ sous-onglets Classe/Cosmétique + pilules Quêtes/Succès/Amis/Clan + boutons Social + card-art modes Arène (à vérifier en Éditeur) = manques sans asset correspondant.
- **🏁 EXPANSION SUCCÈS CLÔTURÉE À 100 (décision Lorenzo 26 mai)** : on s'arrête à **100 succès** (au lieu de viser 200). Familles couvertes : premiers pas, combat (compteurs + maîtrise par classe + stats + séries de victoires + prestige), progression (niveaux + polymathe), ranked, économie, collection, engagement (quêtes + BP). Les 200 de la roadmap V1 sont **révisés à 100** ; si on veut plus tard pousser au-delà, les pistes restantes sont défis saisonniers/hebdo, catégories menu dédiées (build client), paliers numériques additionnels. **BP = 100 tiers curés + pacing validé.** ⇒ Chantier succès + BP **terminé.**
- **📊 AVANCEMENT SUCCÈS : 100 / 100 (cible révisée).** Les batches "pur backend naturels" (progression/niveaux, ranked, économie, collection, engagement) sont **épuisés** ; les succès de combat (87) sont posés mais dormants. Pour aller plus loin : (a) **S-STATS** = le client Unity envoie `MatchCombatStats` (dégâts/sorts/victoire parfaite/tours) → succès de combat [touche Unity] ; (b) **séries de victoires** [migration champ `Profile`] ; (c) plus de **paliers numériques** sur compteurs existants [backend, peu de saveur] ; (d) bascule sur **BP-A** (re-design récompenses BP). ⚠️ Catégorie dédiée menu (Classé/Économie/etc.) = petit build client (tout sous 'Combat'/'Progression' en attendant).
- **Brique BP-A — Re-design des récompenses Battle Pass ✅ DÉPLOYÉ PROD (commit `da27ce0`)** : remplace le filler procédural par une courbe **curée** sur 100 tiers. **Gratuit utile** = Nymos croissant par tranche (25/40/50/60) + cosmétiques accessibles aux jalons 20/50/80/100 (bannière/titre de saison). **Premium** = ligne cosmétique **exclusive** tous les 10 paliers (emote/bannière + skin mi-saison T50 + skin légendaire T100) + Nymos majorés (50/65/80/100, bonus +25 aux paliers ×5). `BpReward.cosmeticId` = slots réservés **DESIGNER-FIRST** : `claimTier` octroie un `UserCosmetic` si l'id existe au catalogue boutique, sinon log différé (en attente Kyami) ; Nymos inchangés (idempotents). Test `npm run test:bp` (structure curée + claims + verrous tier_locked/already_claimed/premium_locked + non-octroi des slots designer-first) → **PASSED E2E prod**. Le test confirme le wiring croisé : `addBpXp(25000)` débloque le succès d'engagement `bp_tier_10`. Pas de migration. **Aucune modif client** (UI BP affiche déjà `tiers` du `GET /battlepass/me`).
- **Batch contenu #6 — Séries de victoires ✅ DÉPLOYÉ PROD (commit `8ec9118`)** : `Profile.currentWinStreak` + `bestWinStreak` (migration `20260526010000_profile_win_streak`). `onMatchEnd` → `updateWinStreak(userId, isWin)` : victoire +1 / défaite reset 0 / égalité intacte ; déverrouille `win_streak_3/5/10/15/20` (unlock permanent) ; skip si pas de Profile (contexte non-ranked). **+5 succès** (catégorie `combat` ; Invaincu T20 = slot cosmétique titre placeholder) → catalogue **87 → 92**. Validé E2E prod (3 victoires → ws3, reset sur défaite avec best=3 conservé, 5 victoires → ws5, best=5). En prod les séries sont de fait ranked-only (le settle ranked exige déjà un Profile). **Aucune modif client.**
- **Validation pacing XP Battle Pass (26 mai) — AUCUN changement de code** : saison 90j, pass = 100 000 XP (100×1000). Sources : match classé (V 200/N 150/D 100), quotidiennes 900 XP/j (reset journalier), hebdo 2500 XP/sem, saisonnière 5000. Scénarios : casual ~32k (≈tier 32, finit jamais — normal), **régulier ~93k (≈tier 90+, cible idéale)**, hardcore ~172k (fini ~jour 45-50). **Point notable** : quêtes seules ≈ 118k > 100k → un complétionniste boucle le pass sans XP de match (≈jour 70). Jugé **OK pour l'alpha** (généreux = rétention/early adopters). **Levier si resserrage voulu plus tard** : −25 % sur l'XP des quotidiennes (900→~675/j) ⇒ quêtes-seules ≈ 90k. Non appliqué.
- **⚙️ NOUVEAU MODE DE TRAVAIL (Lorenzo 26 mai)** : Lorenzo **ne fait plus rien en local** côté backend → **Claude gère commit + push + deploy OVH + validation prod** directement (cf [[feedback-claude-handles-backend-deploy]]). Flux : commit `main` → `git push origin main` → `ssh ubuntu@149.202.57.68 "cd /opt/nymora-backend && ./scripts/deploy.sh"` → test via service `migrator` (`--profile tools run --rm migrator npx ts-node src/scripts/<test>.ts`).
- **PROCHAIN STEP : à décider** — S-STATS ✅ + BP-A ✅ + séries de victoires ✅ faits (92/200). Les familles « riches » backend sont posées. Pour aller plus loin vers 200 : (a) **plus de paliers numériques** sur compteurs existants [backend, peu de saveur — gonfle vite le compte mais répétitif] ; (b) **succès saisonniers / défis hebdo** [nécessite design + ancrage période] ; (c) **catégories dédiées menu** (Classé/Économie/Séries/…) = petit build client Unity. Côté BP : valider le **pacing XP** (gain par win/quête vs 100k XP/saison) si on veut ajuster la cadence. ⚠️ Reste à pousser : commit Unity local `8f0f6d3` (S-STATS.b) en fin de session.

---

**🔒 DÉCISION SCOPE — 26 mai 2026 (Lorenzo + Kyami) : PRÉ-ALPHA = 1v1 UNIQUEMENT.** Les modes **2v2 / 3v3 sont RETIRÉS du scope pré-alpha** et reportés dans une **future roadmap post-alpha** (à créer plus tard, PAS maintenant). Raison : petite base joueurs → ouvrir 3 files de matchmaking (1v1=2 / 2v2=4 / 3v3=6) éclaterait la liquidité, temps d'attente trop long pour trouver une game. Le 1v1 (+ IA solo hors-ligne en fallback) reste le seul mode compétitif de la pré-alpha. L'archi reste compatible (les scènes ranked sont des clones du casual) → ajout sans dette technique plus tard. ➡️ **Ne plus considérer 2v2/3v3 comme un chantier pré-alpha.** Renverse la décision du 16 mai (« 2v2/3v3 maintenus dans le scope alpha »).

---

**SESSION 26 mai 2026 bis (🛡️ BANDEAU DE CLAN ORNEMENTAL + FIXES LEURRES/POP-UPS) :** grosse session UI hub. Client Unity **commité + poussé** (5 commits `6159ca9..6cd14dc` sur `origin/main`, repo `DoctorL08/Nymora`). Backend **commité + poussé + DÉPLOYÉ EN PROD** (`1f0b275`, migration appliquée). 100% View/UI/hub → **pas de bump CombatRulesVersion**.

- **Fix leurres Ghostra** (combat, View) : (1) tri iso dynamique `sortingOrder = 1000-(x+y)*10` comme `CombatantView` → un perso derrière un leurre ne passe plus devant ; (2) tooltip ancré sur le leurre **précis** survolé → chaque leurre a son propre tooltip (avant : 1 seul collé au 1er leurre). Validé Lorenzo.
- **Restyle pop-ups notification** (`RestyleNotificationPopupsTool`, `Nymora > Setup > UI Menu > Restyle Notification Popups`) : défi/ami/clan à la DA menu (arrondi, Ari, boutons Accepter primaire / Refuser ghost). Défi épinglé haut-droite + agrandi + boutons **côte à côte** (étaient empilés/coupés) ; ami recalée sur la pop-up clan. **Scène `10_CommunityHub` modifiée** → designer rebuild standalone.
- **🎖️ Bandeau de clan ornemental "ruban"** (feature complète bout-en-bout) :
  - **Pack SVG** (`Resources/UI/Icons/Clan/` + `Banner/`, générés par `tools/generate_clan_svgs.py` — **hors repo Unity**, relançable) : 10 formes + 31 emblèmes ; bandeau = **8 bouts de ruban** (pennon/taper/scroll/rounded/forked/bevel/flare/studded) + 8 fioritures de nom + 5 médaillons (médaillons **non utilisés**, concept central abandonné). Tous `svgType:1`, blancs teintables. Fenêtres d'aperçu : `Clan Crest Preview` + `Clan Banner Preview`.
  - **Tooltip avatar refondu** (`HubAvatarHoverTooltip`) : plaque rectangulaire englobant clan/pseudo/titre + bouts de ruban (~½ hauteur) + fioritures autour du nom + contour TMP + **snap pixel** (anti-shimmer au mouvement). Métriques dérivées de la hauteur de texte rendue (facteurs en tête de fichier).
  - **Backend (prod)** : `Clan.bannerEnd`/`bannerFlourish` (+ `bannerColor` = couleur ornement) ; `PATCH /clans/me` (chef) ; `GET /clans/banner-by-name/:name` (fetch tooltip) ; `formatClan` renvoie les pièces. Migration `20260526000000_clan_banner_pieces` appliquée.
  - **Client** : `ClanDto` étendu + `ClanBannerDto`/`ClanUpdateBody` ; `UpdateClanBannerAsync`/`GetClanBannerByNameAsync` (+ `PatchJsonAsync`). **Builder repliable** dans menu Social→Clan (chef) : aperçu live (clan/Pseudo/titre génériques) + sélecteurs bout/fioriture + pastilles couleur + Sauvegarder → `PATCH` + invalide cache tooltip. **Archi = fetch+cache, zéro `[Networked]` → pas de regen.**
- **⚠️ À RETENIR** : (1) le tooltip résout les settings backend via `Resources.FindObjectsOfTypeAll<NymoraBackendSettings>` (SO déjà chargé par les panneaux) + token `HubChatClient.DevToken` ; (2) `tools/generate_clan_svgs.py` vit hors du repo Unity (le déplacer dedans si on veut le versionner) ; (3) `QuantumMap.asset` toujours laissé non commité (piège bake). 
- **PROCHAIN STEP** : à décider (commits Unity à venir = brique par brique ; polish bandeau, autres pièces SVG, ou 2v2/3v3).

---

**SESSION 26 mai 2026 (🎨 RE-SKIN HUD COMBAT (DA hub) B1→B8 + CHAT EN COMBAT + FIX CADRE NOIR) :** le HUD combat passe à la DA du menu hub (monochrome + police Ari + coins arrondis), le chat suit le joueur du hub jusqu'en combat, et un reliquat visuel bas-gauche est corrigé. **100% View → pas de bump CombatRulesVersion.** Tout commité **localement** (push GitHub en fin de session). ⚠️ Scènes combat modifiées → **designer doit rebuild son standalone**.

- **Re-skin HUD combat B1→B8** (tool `Nymora > Setup > UI Menu > Restyle Combat HUD`, idempotent, sur les 3 scènes combat en lisant les refs du `CombatHUDController`) — **8 commits `feat(combat-ui)`** : B1 `7c91593` barre de sorts + bouton Fin de tour ; B2 `ca94944` panneaux ressources P0/P1 ; B3 `31ea999` timer + n° de tour + bandeau « ton tour » (+ chip sombre arrondi sur le fond timer) ; B4 `368602d` timeline d'initiative + **retrait fond noir** ; B5 `01bab75` tooltips sorts + passif ; B6 `5ecc778` overlay fin de match + bouton Abandonner ; B7 `a889ce8` panneau passif ; B8 `4fdda7d` intro pile-ou-face (police Ari, pièce bleu/orange conservée car fonctionnelle). Re-skin **bouclé**.
- **Chat en combat** (commit `f83dd1c`) : `AddCombatChatTool` (`Nymora > Setup > UI Menu > Add Chat to Combat Scenes`) instancie le chat du hub dans `30_CombatIA` / `33_CombatCasual` / `40_CombatRanked1v1` (Canvas dédié, ancré bas-gauche, **clé de persistance dédiée `ChatPanelCombat`** + taille 450×310 fixée — sinon le panneau retombait sur la taille prefab minuscule = fine barre). Le `HubChatUI` se reconnecte au `HubChatClient` (DontDestroyOnLoad) + affiche le `ChatFeed` static → **la conversation hub→combat suit**. **Garde clavier** (`CombatInputController.IsTypingInInputField`) : quand un `TMP_InputField` a le focus, les hotkeys de sorts 1-7 sont suspendues (sinon taper « 1 » armait le sort 1). **Validé Lorenzo.**
- **Fix cadre noir bas-gauche** (commit `ae229d3`) : `PassiveTooltipView.EnsureUI()` créait le `Panel` actif (défaut Unity) et `Hide()` en Awake ne le désactivait pas (garde `if (_visible)` = false au démarrage) → tooltip affichée **à vide** (hauteur = padding seul ~28px) parquée à `(0,0)` = coin bas-gauche = « bout de cadre noir transparent ». Fix : `SetActive(false)` du panneau **dès sa création**. Script seul → pas de rebuild standalone pour ce point. Ajouté aussi `DiagnoseBottomLeftUiTool` (`Nymora > Validation > Diagnose Bottom-Left UI`) : diag read-only des Graphic UI occupant le coin bas-gauche (Play Mode).
- **⚠️ À RETENIR** : (1) `Assets/QuantumUser/Resources/QuantumMap.asset` a vu son `ScenePath` réécrit (40_Ranked → 30_IA) **comme artefact de bake** quand les tools ont ouvert les scènes — **laissé non commité** (sans rapport, lié au piège [[project-quantum-map-per-combat-mode]]). À revert ou conserver selon le WIP Quantum/IA. (2) Le `ChatPanelCombat` est **voulu** en combat — ne pas le confondre avec un reliquat. (3) Re-skin combat = 100% View, relançable via le tool sans risque sim.
- **PROCHAIN STEP** : à décider (2v2/3v3, skin en combat 5.10 quand Kyami livre les stages, ou autre polish).

---

**SESSION 25-26 mai 2026 (🧹 POST-M8 — NETTOYAGE UI + AFFICHAGE + ICÔNES SVG + CHAT) :** polish après la clôture du menu Échap. 100% View, pas de bump CombatRulesVersion. Commits locaux.

- **Nettoyage ancienne UI** : `HubLegacyUiCleanupTool` (`Nymora > Setup > UI Menu > Hide/Restore Legacy Hub Buttons`) masque les 8 boutons legacy (Arène/Deck/Profil/Amis/Clan/Quêtes/BattlePass/Boutique) — les `*Panel` (logique réutilisée par le menu) restent. **Bouton AUDIO** : `AudioSettingsPanel` ne se crée plus dans le hub (`scene.Contains("CommunityHub")` skip) — l'audio est dans le menu ; conservé sur 00_Login.
- **Devise ash/blood** : devise persistante posée sur le canvas menu (toujours visible, `HubMenuShell.BuildPersistentCurrency`), icônes **ash=Nymos / blood=Shards** (PNG dans `Resources/UI/Icons/ui_icon_nymos|shards.png`), fetch `/wallet/me` + push WS ; **ancien `HubWalletWidget` masqué** au Start (plus de doublon). Câblé aussi sur les boutons d'achat boutique + bandeau « Possédé » (check).
- **Affichage enrichi** (`DisplaySettingsController`, persistant DontDestroyOnLoad, PlayerPrefs) : Mode d'affichage (Plein écran / **Sans bordure** / Fenêtré), Résolution (liste moniteur + recommandé=native), Luminosité (**assombrissement seul**, overlay), VSync, Limite FPS (30/60/120/144/Illimité), Effets visuels=toggle post-process URP (`renderPostProcessing` sur toutes caméras, ré-appliqué par scène). UI = sélecteurs ‹ › + sliders dans `HubMenuSettings` onglet Affichage.
- **Icônes SVG** : package **`com.unity.vectorgraphics 2.0.0-preview.24`** ajouté ; 13 icônes line monochrome dans `Resources/UI/Icons/` (`ui_icon_*`), **importées en `svgType:1` Textured Sprite** (sinon invisibles dans Image UGUI). Câblées : 5 onglets barre haute, succès (check/cadenas), Battle Pass (coin check/cadenas), onglets cosmétiques. Loader `HubMenuUIFactory.LoadIcon`.
- **Chat** : `HubChatRestyleTool` (`Nymora > Setup > UI Menu > Restyle Chat`) applique la DA (police Ari, monochrome, arrondis). `UiResizableWindow` = **déplaçable + redimensionnable** (poignée haut-droite, persisté PlayerPrefs, **confiné à l'écran** via coins monde). **Feed persistant entre scènes** (`ChatFeed` static). **Onglet Clan** (canal `clan:<clanId>` via `HubChatClient.JoinChannel` + `HubClanPanel.MyClanId`) — ⚠️ **dépend du backend** (relai du canal clan à valider/compléter côté Node). **Son notif = MP reçu uniquement** (plus de son sur global/envoi).
- **⚠️ Reliquats / à retenir** : (1) chat clan = code prêt, **backend à confirmer** (JOIN_CHANNEL + broadcast `clan:*`). (2) Manifeste icônes dans `_docs/UI_ICONS_NEEDED.md`. (3) Tools de scène (legacy hide, restyle chat) → scènes `10_CommunityHub`/`00_Login` modifiées, **designer doit rebuild standalone**.

---

**SESSION 25 mai 2026 (🎨 REFONTE UI — MENU « ÉCHAP » MODERNE, M0→M8 ✅ BOUCLÉ) :** gros chantier UI démarré : remplacer les boutons épars du hub par un menu moderne minimaliste ouvert par **Échap + bouton hamburger ☰** (haut-gauche). 100% View, **pas de bump CombatRulesVersion**. Commits Unity **locaux** (push en fin de session). Cf mémoire [[project-menu-refonte-ui]] pour les détails/décisions.

- **Fichiers** (`Assets/_Nymora/Scripts/Hub/Menu/`) : `HubMenuTheme` (ScriptableObject palette monochrome + métriques + fonts Ari), `HubMenuUIFactory` (fabrique de widgets : carte/onglet/bouton/panneau + sprite arrondi par code), `HubMenuShell` (coquille : Échap+hamburger, fond sombre 0.82 SANS flou, barre d'onglets, 4 cartes, monnaies haut-droite, écran Personnage), `HubMenuDeckBuilder` (deck builder complet), `HubMenuCosmetics` (inventaire cosmétiques). Tool : `Nymora > Setup > UI Menu > Create or Refresh Menu Shell` (pose `HubMenuCanvas`, assigne theme/backend/classDefs/SpellCatalog/skins/cardArt). Asset thème : `ScriptableObjects/Settings/HubMenuTheme.asset`.
- **M0 kit + M1 coquille** : Échap/hamburger ouvre/ferme (Échap ferme tout), hub assombri + clics bloqués, barre d'onglets (Social/Progression/Paramètres/Report bug/Déconnexion), 4 cartes Accueil (Arène/Personnage/Battle Pass/Boutique) avec illustrations, monnaies Nymos/Shards haut-droite (réutilise Wallet).
- **M2 Arène** : 4 cartes (Entraînement→IA / Ranked 1v1→**matchmaking fenêtre stylée** réutilisant les events file de `HubChatClient` / 2v2-3v3 grisés) + bouton **Classement** → écran 3 onglets 1v1/2v2/3v3 (1v1 = vraies données via `HubLeaderboardPanel.GetLeaderboardTextAsync`).
- **M3 Personnage** : style « hero menu ». Colonne gauche = **sprite idle animé** (reflète le **skin équipé**, boost ×1.4) + **pseudo gros centré** + **titre équipé** (italique doré, depuis `HubAvatar.NetTitle`) + switch **‹ Classe ›** (chevrons = change classe live) + **Niv. N** + barre XP. Panneau unifié + séparateur vertical. Onglet **Classe** = deck builder reconstruit (liste decks + nom + Save/Modifier/Supprimer + Nouveau + 6 slots + onglets Offensifs/Tactiques/Survie + 5 sorts/ligne + **tooltip sort au survol** + encarts Passif[paliers/bonus]/Signature) ; sync combat via `HubDeckBuilderPanel.SetActiveDeckAsync`. Onglet **Cosmétique** = inventaire (onglets Skins/Familiers[clé `pet`]/Titres/Bannières, filtre par classe, équiper/déséquiper).
- **M4 Social** (`HubMenuSocial.cs`, instancié par `HubMenuShell.BuildSocial`) : écran dans un **panneau centré 1040px** (la liste vit dedans, scroll clippé — sinon ça bave pleine largeur sur le hub). 2 sous-onglets pilule **Amis / Clan**. *Amis* = barre Ajouter (input pseudo) + 3 sections (Mes amis [pastille online + MP + Retirer] / Demandes reçues [Accepter/Refuser] / Demandes envoyées). *Clan* = sans clan (créer + invitations reçues) / avec clan (bannière couleur + membres + actions selon rôle FR Chef/Officier/Membre/Recrue : promote/rétrograder/exclure, inviter, quitter/dissoudre). Réutilise 100% l'API `NymoraApiClient` des panneaux `HubFriendsPanel`/`HubClanPanel` (zéro endpoint dupliqué). **Pastille online** = rond parfait (sprite cercle généré) + tooltip survol « Connecté »/« Déconnecté » (calqué sur le tooltip sort du deck builder) ; statut live via events WS `OnFriendsOnlineList/Online/Offline` + snapshot via nouvel accessor `HubFriendsPanel.IsFriendOnline`. Désabonnement auto au Destroy du holder (`DestroyNotifier`).
- **M5 Progression** (`HubMenuProgression.cs`, instancié par `HubMenuShell.BuildProgression`) : même moule que Social (panneau centré 1040px). 2 sous-onglets pilule **Quêtes / Succès**. *Quêtes* = sections par période (Quotidiennes/Hebdomadaires/Saisonnières), ligne = titre (haut-gauche) + récompense `◆ Nymos / +XP` (haut-droite) + **« x/y » sur sa propre ligne** sous le titre (sortir de la barre sinon illisible) + barre en bas + bouton **Réclamer** (si complétée & non réclamée → SFX `RewardClaim`) sinon tag « En cours »/« ✓ Réclamée ». *Succès* (lecture seule, pas de claim) = en-tête `points · débloqués/total` + liste groupée par catégorie (Premiers pas/Combat/Progression), ✓ vert ou ○ + barre, points en or si débloqué, **« x/y » intégré à la description** (`desc · 3/5`). Réutilise `GetQuests`/`ClaimQuest`/`GetAchievementsCatalog`/`GetAchievementsMe`.
- **M6 Paramètres** (`HubMenuSettings.cs`, instancié par `HubMenuShell.BuildSettings`, panneau centré 760px) : 2 sous-onglets pilule **Affichage / Audio**. *Affichage* = toggle Plein écran (`Screen.fullScreen`). *Audio* = 5 sliders montés en code (Volume général/Musique/Effets/Ambiance/Interface) + % live → `NymoraAudioManager.SetBusVolume` (persisté PlayerPrefs). Sliders + toggle 100% code (track/fill/handle, case+coche).
- **M7 Battle Pass + Boutique** (`HubMenuShop.cs` + `HubMenuBattlePass.cs`, instanciés par `HubMenuShell.BuildShop`/`BuildBattlePass`, panneaux quasi plein écran). *Boutique* = vitrine : grille de **cartes centrées** groupées par catégorie (Skins/Titres/Bannières/Emotes), cadre teinté rareté + lueur, grand visuel, nom (wrap+ellipse), boutons d'achat par devise (◆/◇) ou « ✓ Possédé » ; `BuyItemAsync` + SFX `PurchaseSuccess`. *Battle Pass* = **scroll horizontal** de paliers, gouttière GRATUIT (haut)/PREMIUM (bas), cellules **256px** (remplissent la hauteur), états Réclamé/Réclamer(cliquable)/Verrouillé/Premium, palier courant surligné ; `ClaimBattlePassTierAsync` + SFX `RewardClaim`. Réutilisent l'API de `HubShopPanel`/`HubBattlePassPanel` (zéro endpoint dupliqué).
- **⚠️ Coins panneaux** : tentative `RoundedSprite(28)` + liseré clair **abandonnée** (Lorenzo a demandé de revenir en arrière) ; les panneaux restent en `RoundedSprite(28)` simple — coins peu visibles (sombre sur voile sombre) mais accepté en l'état. Ne pas re-litiger sans demande.
- **⚠️ Piège résolu (sizeDelta défaut 100×100)** : un `RectTransform` créé par code + ancrages étirés H = largeur parent **+100** → débord 50px de chaque côté (texte/boutons coupés). Fix : forcer `content.sizeDelta = Vector2.zero` sur le contenu scrollable. ⚠️ `HubMenuCosmetics` (M3c) utilise le même pattern de scroll — à corriger si débord constaté.
- **Art cartes** : PNG dans `Assets/_Nymora/Art/UI/MenuCards/` (512×960, ratio 8:15), masque arrondi pour épouser le contour. Mapping dans `HubMenuShellTool.CardArtMap`. Entraînement=ui_image_colossar, Ranked1v1=ui_image_soulrender.
- **M8 barre haute** ✅ (`HubMenuShell.BuildReport`/`BuildLogout`/`DoLogout`) : *Report bug* → `Application.OpenURL("https://discord.gg/haRtXKCERx")` + écran info (bouton « Ouvrir le Discord »). *Déconnexion* → dialogue de confirmation → `AuthService.Logout()` (efface JWT PlayerPrefs) + `Destroy(HubChatClient.Instance.gameObject)` + `SceneTransition.Load("00_Login")` (le NetworkRunner Fusion meurt avec la scène hub). Helper `BuildCenterDialog`.
- **PROCHAIN STEP : nettoyage ancienne UI** — retrait progressif de l'ancien Deck Builder + boutons hub épars (Battle Pass/Quêtes/Boutique/Profil/Amis/Clan/Decks/Arène en bas + chat) maintenant que le menu Échap couvre tout. Le chantier menu M0→M8 est **bouclé et validé**. (Tous les écrans réutilisent l'API existante, zéro endpoint dupliqué ; Classement reste dans l'Arène M2.)
- **À RETENIR** : familiers = clé type `"pet"` (✅ confirmé Lorenzo 25 mai) ; `ui_image_ghostra/necram/nightseer` restés en 150×300 (non utilisés) ; relancer le tool après tout ajout d'asset (classDefs/skins/cardArt).

---

**SESSION 24 mai 2026 (✨ POLISH — ANIMATIONS D'OUVERTURE/FERMETURE DES PANNEAUX HUB) :** les 11 panneaux du hub s'ouvrent/se ferment en pop + fondu au lieu d'un toggle sec. Validé Play Mode (ouverture puis fermeture). Commit Unity **local** (push GitHub en fin de session). **Pas de bump CombatRulesVersion** (100% View).

- **Composant `UiPanelAnimator`** (`Assets/_Nymora/Scripts/Hub/UiPanelAnimator.cs`, namespace `Nymora.Hub`) : posé sur le `_panelRoot` d'un panneau. **Ouverture auto** via `OnEnable` (fondu + léger zoom `StartScale=0.96→1`, `Duration=0.18s`, ease-out cubic, temps non-scalé) — aucun changement de code, les panneaux continuent d'appeler `_panelRoot.SetActive(true)`. **Fermeture opt-in** : `UiPanelAnimator.CloseAnimated(_panelRoot)` joue le fondu sortant PUIS `SetActive(false)` (fallback robuste si pas d'animator/déjà inactif). Self-provisioning du CanvasGroup.
- **Outil d'attache** `SetupHubPanelAnimationsTool` (`Nymora > Setup > Setup Hub Panel Animations`) : détection générique par réflexion (tout MonoBehaviour scène avec un champ `_panelRoot` GameObject) → attache l'animator sur les 11 panneaux d'un coup, idempotent. **Lancé + scène 10_CommunityHub sauvegardée** par Lorenzo.
- **11 `Close()` routés** vers `CloseAnimated` (`HubArenaPanel`, `HubClassSelectorPanel`, `HubFriendsPanel`, `HubProfilePanel`, `HubClanPanel`, `HubLeaderboardPanel`, `HubShopPanel`, `HubQuestsPanel`, `HubBattlePassPanel`, `HubRankedSearchPanel`, `HubDeckBuilderPanel`). Effets de bord (annuler la file matchmaking Ranked, restaurer boutons Arène/Wallet du Deck) restent **immédiats** ; seul le masquage attend le fondu. Tous dans namespace `Nymora.Hub` → pas de `using` ajouté.
- **À RETENIR** : (1) réglages anim en `const` dans `UiPanelAnimator` (durée/échelle). (2) Edge mineur connu : rouvrir un panneau dans les ~0.18s de son fondu de fermeture peut quand même le fermer (l'`Open` SetActive(true) sur GO déjà actif ne re-déclenche pas `OnEnable`) — cas rare, non durci. (3) L'animator scale tout le `_panelRoot` ; zoom volontairement subtil (0.96) pour éviter un liseré du hub derrière un panneau plein écran.
- **PROCHAIN STEP** : chantier polish suivant à décider (SFX + musique combat dispo sans Kyami ; juice combat ; feedback ranked ; notes de version launcher).
...
- **Écran de chargement épuré (décision Lorenzo)** : PAS d'astuces rotatives / barre de progression — les chargements vont trop vite pour qu'on ait le temps de lire. On garde voile noir + roue seuls. Chantier transitions **clos**.
- **PROCHAIN STEP** : chantier polish suivant à décider (SFX + musique combat dispo sans Kyami).
