...
**Phase actuelle :** 🏆 **PHASE 3 NECRAM CLÔTURÉ** — 4/5 classes complètes (Soulrender + Nightseer + Colossar + Necram). **Ghostra prochaine** (Bloc D Phase 3) — décision audit 16 mai 2026 (cf memory `project_roadmap_v1_amendments_may16`).
**Brique en cours :** **GHOSTRA Phase 3 Bloc D** — assets dispo `C:\Users\Lorenzo\Downloads\ghostra\Ghostra` (avatar + 3 stages × 2 dir × 6 anims + 17 icônes sorts + marque ciblage + tile case voilée + VFX signature 8 frames). Mécaniques uniques : Rémanence (ressource) + Angle Mort (passif dorsal) + 3 leurres indiscernables + permutations 0 PA + Exécution Spectrale signature.
