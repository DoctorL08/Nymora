...
**Dernière mise à jour :** 15 mai 2026 (session nuit, plus tard) — ✅ **5.1 NIVEAUX PAR CLASSE VALIDÉE** (E2E OK). Première brique Phase 5 livrée. Backend `ClassProgression` Prisma + 2 endpoints `/progression/me` et `/progression/award-xp` + courbe XP `100 + n*100` + level cap 50 + push WS `XP_AWARDED` / `CLASS_LEVEL_UP` depuis route (single source of truth). Unity onglet "Classes" du profil branché (placeholder remplacé par 5 rows avec level + XP bar + couleur classe). Award XP TEMP MVP wiré sur `HubMatchResultDisplay` au retour hub (V=50 / D=15 / Draw=25 XP). Smoke test 10 scénarios → PASSED. **Convention sauvegardée** : XP de classe sera ranked-only en prod (Phase 6), wiring temporaire à retirer.  
