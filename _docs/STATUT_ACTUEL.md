...
**SESSION 25 mai 2026 (🎨 REFONTE UI — MENU « ÉCHAP » MODERNE, M0→M3c) :** gros chantier UI démarré : remplacer les boutons épars du hub par un menu moderne minimaliste ouvert par **Échap + bouton hamburger ☰** (haut-gauche). 100% View, **pas de bump CombatRulesVersion**. Commits Unity **locaux** (push en fin de session). Cf mémoire [[project-menu-refonte-ui]] pour les détails/décisions.
...
- **PROCHAIN STEP : M4 Social** (onglet Social barre haute : Amis/Ajouter/Demandes + sous-onglets Clan + Classement). Puis M5 Progression (Succès+Quêtes), M6 Paramètres (NEW), M7 Battle Pass+Boutique restylés, M8 barre haut (Report bug, Déconnexion→00_Login). Ancien Deck Builder + boutons hub épars encore présents → retrait progressif.
- **À RETENIR** : familiers = clé type `"pet"` à confirmer backend ; `ui_image_ghostra/necram/nightseer` restés en 150×300 (non utilisés) ; relancer le tool après tout ajout d'asset (classDefs/skins/cardArt).
