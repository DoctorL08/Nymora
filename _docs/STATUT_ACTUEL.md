...
**SESSION 25 mai 2026 (🎨 REFONTE UI — MENU « ÉCHAP » MODERNE, M0→M6) :** gros chantier UI démarré : remplacer les boutons épars du hub par un menu moderne minimaliste ouvert par **Échap + bouton hamburger ☰** (haut-gauche). 100% View, **pas de bump CombatRulesVersion**. Commits Unity **locaux** (push en fin de session). Cf mémoire [[project-menu-refonte-ui]] pour les détails/décisions.
...
- **PROCHAIN STEP : M7 Battle Pass + Boutique** restylés (panneaux existants `HubBattlePassPanel`/`HubShopPanel` à reprendre). Puis M8 barre haut (Report bug, Déconnexion→00_Login). Ancien Deck Builder + boutons hub épars encore présents → retrait progressif. (M4 Social + M5 Progression + M6 Paramètres = ✅ livrés + validés ; Classement reste dans l'Arène M2.)
