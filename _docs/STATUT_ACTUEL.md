...
**SESSION 25 mai 2026 (🎨 REFONTE UI — MENU « ÉCHAP » MODERNE, M0→M4) :** gros chantier UI démarré : remplacer les boutons épars du hub par un menu moderne minimaliste ouvert par **Échap + bouton hamburger ☰** (haut-gauche). 100% View, **pas de bump CombatRulesVersion**. Commits Unity **locaux** (push en fin de session). Cf mémoire [[project-menu-refonte-ui]] pour les détails/décisions.
...
- **PROCHAIN STEP : M5 Progression** (Succès + Quêtes). Puis M6 Paramètres (NEW), M7 Battle Pass+Boutique restylés, M8 barre haut (Report bug, Déconnexion→00_Login). Ancien Deck Builder + boutons hub épars encore présents → retrait progressif. (M4 Social = ✅ livré + validé ; décision : Classement reste dans l'Arène M2, pas dupliqué dans Social.)
