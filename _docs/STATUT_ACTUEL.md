...
**Dernière mise à jour :** 16 mai 2026 (session jour, suite 2) — ✅ **3.5.c.iv PULSE SANGUIN VERT VALIDÉ** (E2E OK) + ✅ **3.5.c.iii DRAIN VITAL VALIDÉ** (E2E OK). 4e brique Bloc C Necram survie → **Bloc C SURVIE 4/5** (reste Cocon Putride signature). **Pulse Sanguin Vert** : 3 PA self, heal Necram caster base 70 + 15/marque venin somme sur ennemis vivants Manhattan ≤4 (cap bonus +90 HP). +30 HP additionnel avec 1 PT optionnel (Shift+X). Marques NON consommées. Touche AZERTY **X** context-aware (Nightseer Pas Furtif Shift=1PR / Necram **Pulse Sanguin Vert** Shift=1PT). `CombatRulesVersion 33→34`. SpellId.NecramPulseSanguinVert=83. Itère `f.Filter<Combatant>()` pour somme stacks ennemis dans rayon Manhattan. Test E2E validé : base 70+30 (2 marques) sans PT → 100 HP ; base 70+30+30 (PT) → 130 HP, consume 1 HG/PT ; base 70+60+30 (4 marques + PT) → 160 HP ; rejet PA 2<3 ; non-régression Pas Furtif Nightseer. **Drain Vital** : 3 PA range 4, 60 dmg cible + heal caster 30 HP (ou 60 HP si target.VeninStacks≥3). Marques NON consommées. Heal applique même si target meurt (Bible "le siphon"). Touche AZERTY **N** context-aware (Nightseer Sève Sauvage self / Necram **Drain Vital** target ennemie). `CombatRulesVersion 32→33`. SpellId.NecramDrainVital=82. Test E2E validé : 60 dmg + heal 30 si target 0 marques ; 60 dmg + heal 60 si target 4 marques (double-cast consécutif +120 HP total) ; rejets propres range/filter/case vide ; non-régression Sève Sauvage Nightseer. **Total Necram : 14/16** sorts livrés (5 offensifs + 5 tactiques + 4 survie). Reste : Cocon Putride (3.5.c.v signature panic 1×/match <30% HP) + Virus Fatal (signature ultime 6/6 PT).  
...
**Phase actuelle :** 🚧 **PHASE 3 NECRAM REPRISE EN PARALLÈLE** (Bloc C survie Necram en cours, 2/5 livrés ce soir) — Décision implicite Lorenzo 16 mai : avancer Necram en parallèle de Phase 5 maintenant que pack visuel + framework venin sont en place.
**Brique en cours :** **3.5.c.v Cocon Putride** (prochain sort survie Necram — signature panic) — Bible : utilisable <30% HP uniquement, 1×/match, heal 220 HP + AoE 1 marque rayon 4. Mécanique once-per-match + gate health requise.
