...
- **🔜 S-STATS.b (Unity, TODO)** : le client calcule les stats en fin de combat (dégâts infligés/subis, sorts lancés, nb de tours = rounds cf [[project-turn-semantics]]) et les envoie dans le payload `report-result`. Touche la sim/View Quantum + le réseau → **build/test combat de Lorenzo requis**. C'est ce qui allumera les 9 succès de combat.
- **📊 AVANCEMENT SUCCÈS : 87 / 200.** Les batches "pur backend naturels" (progression/niveaux, ranked, économie, collection, engagement) sont **épuisés** ; les succès de combat (87) sont posés mais dormants. Pour aller plus loin : (a) **S-STATS** = le client Unity envoie `MatchCombatStats` (dégâts/sorts/victoire parfaite/tours) → succès de combat [touche Unity] ; (b) **séries de victoires** [migration champ `Profile`] ; (c) plus de **paliers numériques** sur compteurs existants [backend, peu de saveur] ; (d) bascule sur **BP-A** (re-design récompenses BP). ⚠️ Catégorie dédiée menu (Classé/Économie/etc.) = petit build client (tout sous 'Combat'/'Progression' en attendant).
...
- **PROCHAIN STEP : à décider** — (a) S-STATS succès de combat [touche Unity], (b) séries de victoires [migration], (c) BP-A re-design récompenses BP [backend], ou (d) plus de paliers numériques [backend]. Cf ligne AVANCEMENT.
