...
**SESSION 25 mai 2026 (🎨 REFONTE UI — MENU « ÉCHAP » MODERNE, M0→M7) :** gros chantier UI démarré : remplacer les boutons épars du hub par un menu moderne minimaliste ouvert par **Échap + bouton hamburger ☰** (haut-gauche). 100% View, **pas de bump CombatRulesVersion**. Commits Unity **locaux** (push en fin de session). Cf mémoire [[project-menu-refonte-ui]] pour les détails/décisions.
...
- **PROCHAIN STEP : M8 barre haute** (Report bug + Déconnexion→00_Login), puis retrait progressif de l'ancien Deck Builder + boutons hub épars → clôture du chantier menu. (M4 Social + M5 Progression + M6 Paramètres + M7 Boutique/Battle Pass = ✅ livrés + validés ; Classement reste dans l'Arène M2.)
