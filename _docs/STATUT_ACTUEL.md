...
**CombatRulesVersion :** **23** (bump 3.5.a.ii — 2 sorts offensifs Necram NecramDetonationVirulente=72 / NecramFauxDecharnee=73 + handler pre-damage Détonation bonus 50/marque + post-damage RemoveAllMarks + tracker fauxDecharneeMarksTotal in-loop AoE Square3x3 + heal post-loop cap +120). Précédent : 22 = bump 3.5.a.i Crachat Acide / Morsure Putride. Précédent : 21 = bump 3.4 Framework Marque Venin Necram (3 champs Combatant `VeninStacks`/`LastVeninTickOnTurn`/`PutrefactionMarksGainedThisTurn` + passif Floraison 3 paliers densité globale + tick venin bypass shield/réduction + regen +10/marque tier 2 + halo toxique rayon 3 tier 2 + cap 4 marques/cible + cap 2 PT/tour Necram via marques appliquées). Avant : 20 = bump 3.3.d Colossar Effondrement signature.
