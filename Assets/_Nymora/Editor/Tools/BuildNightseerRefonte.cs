...
{
    /// <summary>
    /// Refonte graphique NIGHTSEER (15 juin 2026) — pipeline complet en 1 clic.
    /// Calque EXACT de BuildSoulrenderRefonte (lui-meme calque de BuildGhostraRefonte) :
    /// meme modele visuel, memes decisions techniques.
    ///
    /// Modele visuel :
    ...
    /// La montee de stage du Nightseer reste pilotee par CombatantRenderer.ComputeStage via la
    /// RESSOURCE (chemin generique : &lt; 40% cap -> stage0, &lt; max -> stage1, == max -> stage2).
    /// Cote View, c'est CombatantView.ApplyAura qui allume la bonne couche. PAS de leurres/DecoyView.
    ///
    /// Sources (PNG livres par le designer, bandes horizontales, cellules 128×128) :
    ///   Desktop/Nymora_Graph/Nightseer/PNG/
    ///     stage0/Nightseer_base_{idle,walking,attack,cast,hurt,death}_{SE,NE}.png   (12 sheets)
    ///     stage1/Nightseer_stage1_aura_{back,front}_{SE,NE}.png                     (4 sheets)
    ///     stage2/Nightseer_stage2_aura_{back,front}_{SE,NE}.png                     (4 sheets)
    ///   ⚠ Nommage livre : base en MAJUSCULE "Nightseer_base_" (≠ Soulrender minuscule),
    ///     anim de marche = "walking", auras en "Nightseer_stage{n}_aura_...".
    ///
    /// Ce que le tool fait : copie/slice (PPU 96, pivot 0.5/0.1) les 20 PNG -> clips -> 2 controllers
    /// base (NightseerBase_SE/NE, reconstruits EN PLACE -> guid preserve) + 8 sets de frames d'aura
    /// -> bind du prefab Combatant_Nightseer + retarget de l'avatar hub (Nightseer.asset).
    ///
    /// View-only : aucun bump CombatRulesVersion. Idempotent (re-run = ecrase/repointe).
    /// Menu : Nymora > Setup > Build Nightseer Refonte.
    /// </summary>
    public static class BuildNightseerRefonte
    {
        // --- Sources (hors projet) ---
        private const string Src = "C:/Users/Lorenzo/Desktop/Nymora_Graph/Nightseer/PNG";

        // --- Calibration combat verrouillee (cf BuildSoulrenderRefonte) ---
        private const int Cell = 128;
        private const float Ppu = 96f;
        private static readonly Vector2 Pivot = new Vector2(0.5f, 0.1f);

        // --- Destinations projet ---
        private const string DstBaseSheets = "Assets/_Nymora/Art/Sprites/Nightseer/Refonte/Base";
        private const string DstAuraSheets = "Assets/_Nymora/Art/Sprites/Nightseer/Refonte/Aura";
        private const string DstClips = "Assets/_Nymora/Animations/Nightseer/Refonte/Clips";
        private const string DstCtrl = "Assets/_Nymora/Animations/Nightseer/Refonte";
        ...
        public static void Run()
        {
            ...
            {
                EditorUtility.DisplayDialog("Build Nightseer Refonte",
            ...
            foreach (var dir in Dirs)
            {
                ...
                {
                    string fileBase = $"Nightseer_base_{file}_{dir}";
                ...
                string ctrlPath = $"{DstCtrl}/NightseerBase_{dir}.controller";
            ...
            foreach (int stage in AuraStages)
            {
                foreach (var layer in AuraLayers)
                {
                    foreach (var dir in Dirs)
                    {
                        string fileBase = $"Nightseer_stage{stage}_aura_{layer}_{dir}";
            ...
            // ----- hub (avatar = idle/walk SE+NE de la NymoraClassDefinition Nightseer) -----
            RetargetNightseerHub(hubFrames, report);

            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();

            Debug.Log("[BuildNightseerRefonte]\n" + string.Join("\n", report));
            EditorUtility.DisplayDialog("Build Nightseer Refonte",
                string.Join("\n", report) +
                "\n\nView-only : pas de bump version. Lance un combat IA Nightseer et utilise F10 " +
        ...
        // =====================================================================
        // Hub : repointe les frames idle/walk de la NymoraClassDefinition Nightseer
        // =====================================================================
        private const string NightseerClassAsset = "Assets/_Nymora/ScriptableObjects/Classes/Nightseer.asset";

        private static void RetargetNightseerHub(Dictionary<string, Sprite[]> hub, List<string> report)
        {
            var asset = AssetDatabase.LoadMainAssetAtPath(NightseerClassAsset);
            if (asset == null) { report.Add($"⚠ hub : asset introuvable ({NightseerClassAsset})"); return; }
        ...
        private static void SetArray(SerializedObject so, string prop, Sprite[] sprites)
        {
            ...
            if (p == null || !p.isArray) { Debug.LogWarning($"[BuildNightseerRefonte] champ '{prop}' introuvable/non-array."); return; }
        ...
        // =====================================================================
        // Slice + clips (calque BuildSoulrenderRefonte)
        // =====================================================================
...
#pragma warning restore 0618
        ...
        {
            var p = so.FindProperty(prop);
            if (p == null) { Debug.LogWarning($"[BuildNightseerRefonte] champ '{prop}' introuvable sur CombatantView."); return; }
