...
namespace Nymora.Editor.Tools
{
    ...
    /// </summary>
    public static class BuildColossarAnimator
    {
        ...
        [MenuItem("Nymora/Setup/Build Colossar Animator")]
        public static void Run()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                {
                    Debug.LogError($"[BuildColossarAnimator] Fichier introuvable : {AsepritePaths[i]}");
                    return;
                }
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                controllers[i] = ctrl;
                Debug.Log($"[BuildColossarAnimator] Cree {CtrlPaths[i]} (state machine complete)");
            }

            if (!File.Exists(PrefabPath))
            {
                Debug.LogError($"[BuildColossarAnimator] Prefab introuvable : {PrefabPath}");
            ...
            if (prefab == null)
            {
                Debug.LogError($"[BuildColossarAnimator] Impossible de charger : {PrefabPath}");
                return;
            }
            ...
            try
            {
                ...
                if (animator == null)
                {
                    ...
                    Debug.Log("[BuildColossarAnimator] Animator ajoute au prefab.");
                }
                ...
                if (sr != null)
                {
                    ...
                    if (fallbackSprite != null)
                    {
                        ...
                        Debug.Log($"[BuildColossarAnimator] SpriteRenderer fallback sprite : {fallbackSprite.name}");
                    }
                }
                ...
                if (view != null)
                {
                    ...
                    Debug.Log("[BuildColossarAnimator] CombatantView : Animator + 2 controllers stage0 (NE+SE) binds.");
                }
                else
                {
                    Debug.LogWarning("[BuildColossarAnimator] CombatantView introuvable sur le prefab. Bind a faire manuellement.");
                }

                PrefabUtility.SaveAsPrefabAsset(prefab, PrefabPath);
                Debug.Log($"[BuildColossarAnimator] Prefab sauvegarde : {PrefabPath}");
            }
            finally
            {
        ...
        private static void LogClipSetSummary(string asepritePath, ClipSet set)
        {
            ...
            Debug.Log($"[BuildColossarAnimator] {name} clips : {list}");
            int missing = 0;
            ...
            if (missing > 0)
            {
                Debug.LogWarning($"[BuildColossarAnimator] {name} : {missing} clip(s) manquant(s) — fallback Idle. " +
        ...
        private static void SetObjectRef(SerializedObject so, string propertyName, Object value)
        {
            ...
            if (prop == null)
            {
                Debug.LogWarning($"[BuildColossarAnimator] Champ '{propertyName}' introuvable sur CombatantView.");
                return;
            }
